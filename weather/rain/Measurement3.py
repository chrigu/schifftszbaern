...
class Measurement3(object):
    ...
    *******
    ...
    def from_json(cls, position, raster_width, test_field_width, data):
        obj = cls(position, Measurement3.timestring_to_timestamp(data['timestamp']), raster_width, test_field_width)
        obj.data = data['data']
        if 'location' in data:
    ...
    def __init__(self, radar_image, timestamp, data, label_img, forecast=False, url=None):
